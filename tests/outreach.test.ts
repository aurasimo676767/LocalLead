...
import { buildMessagePayload, validateOutreachMessage } from "../lib/messaging/validation";
...
describe("outreach final validation", () => {
  it("rejects generic copy, missing menu, missing QR and CTA-only regeneration", () => {
    ...
    expect(validateOutreachMessage(good, lead, defaultPreferences).valid).toBe(true);
    for (const text of [
    ...
    ]) expect(validateOutreachMessage(text, lead, defaultPreferences).valid).toBe(false);
    expect(validateOutreachMessage(good.replace(/Vi interesserebbe\?/, "Che ne pensate?"), lead, defaultPreferences, [good]).valid).toBe(false);
  });
  it("uses saved context and defaults food menus without inventing events or photo quality", () => {
    ...
    const payload = buildMessagePayload(lead, defaultPreferences, ["recent"], "previous");
